...
const menuitems=[
    ...
    {id:'5',text:'Configuration Settings',icon:<SettingsIcon />, link:'/admin/configsetting'},
]
...
return (
   <Drawer
    ...
    >
  <Box sx={{ padding: '10px 0px 10px 15px', display: 'flex', justifyContent: 'flex-start' }}>
  ...
  sx={{backgroundColor:'#DEF6DB',padding:"10px",borderRadius:'10px',width:"90%",margin:'auto'}}>
  ...
  <Typography fontSize="12px" color="green">Admin</Typography>
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', flexWrap: 'wrap',gap:"50px"}}> {/* Pushes content below to the bottom */}
        <List>
          {menuitems.map((item) => (
            <ListItem
              key={item.id}
              button
              selected={item.text === 'Dashboard'} // Dashboard is active by default
              sx={{
            ...
            >
              <ListItemIcon sx={{ color: item.text === 'Dashboard' ? 'white' : 'green', minWidth: '40px',
              ...
              <ListItemText
                ...
                sx={{
                  '& .MuiTypography-root': {
                    fontSize: '14px',
                    // Removed whiteSpace: 'nowrap', textOverflow: 'ellipsis', and overflow: 'hidden' to allow wrapping
                  },
                 }}
              />
            </ListItem>
          ...
          <ListItem button>
            ...
            <ListItemText
              ...
              sx={{
                '& .MuiTypography-root': {
                  fontSize: '14px',
                  whiteSpace: 'nowrap', // Kept nowrap for Logout if desired
                  color: 'red',
                },
